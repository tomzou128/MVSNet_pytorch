...
parser.add_argument(
    "--interval_scale", type=float, default=1.06, help="the depth interval scale"
)
...
def save_depth():
    # Get the dataset class for test
    MVSDataset = find_dataset_def(args.dataset)
    test_dataset = MVSDataset(
        args.testpath, args.testlist, "test", 5, args.numdepth, args.interval_scale
    )
    TestImgLoader = DataLoader(
        test_dataset, args.batch_size, shuffle=False, num_workers=1, drop_last=False
    )
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            outputs = model(
                sample_cuda["imgs"],  # [B, N, C, H, W], e.g. [1, 5, 3, 1184, 1600]
                sample_cuda["proj_matrices"],  # (B, N, 4, 4)
                sample_cuda["depth_values"],  # (B, D),  [1, 192]
            )
            ...
            for filename, depth_est, photometric_confidence in zip(
                    filenames, outputs["depth"], outputs["photometric_confidence"]
            ):
...
def filter_depth(scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        save_mask(
            os.path.join(out_folder, "mask/{:0>8}_geo.png".format(ref_view)), geo_mask
        )
        save_mask(
        ...
        if args.display:
            ...
            cv2.imshow(
                "ref_depth * mask", ref_depth_est * final_mask.astype(np.float32) / 800
            )
            cv2.waitKey(0)
...
if __name__ == "__main__":
    ...
    for scan in scans:
        ...
        filter_depth(
            scan_folder,
            out_folder,
            os.path.join(args.outdir, "mvsnet{:0>3}_l3.ply".format(scan_id)),
        )
